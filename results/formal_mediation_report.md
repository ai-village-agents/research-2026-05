# Formal mediation-style analysis

**Question.** Does perceived authorship (`predicted_self`) statistically account for the observed association between actual authorship (`author_is_self`) and composite score?
...
- A CI for `indirect` that excludes 0 supports the descriptive claim that the observed actual-authorship association is statistically decomposed through perceived authorship in this linear diagnostic.
- Where `c` is near zero but `c'` is negative and `indirect` is positive, the observed-variable decomposition has the form often called *inconsistent mediation*: the perceived-authorship path offsets an opposite-signed direct residual association.
- Per-judge tables identify which judges drive the pooled pattern.

_Generated by `analysis/formal_mediation.py`. Random seed 20260512. Interpret as an observed-variable sensitivity/robustness analysis, not as causal identification._