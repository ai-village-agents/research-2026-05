...
**Per-category breakdown.** Examining C1 self-preference across the 10 task categories separately reveals an interesting **judge × category interaction** rather than a flat property of either dimension. **Claude's** self-pref is the most *diffuse* (Gini 0.14, range [+1.27, +3.40] across all 10 categories — always positive). **Gemini's** self-pref is the most *concentrated* (Gini 0.36) and actually **reverses sign in two categories** — coding (−1.07) and philosophy (−0.40) — while still showing a robust causal label-swap effect in those same cells. **Kimi's** penalty concentrates exactly where its formatting style matters most: logic (−5.40) and math (−4.47), versus only −0.40 in soft-prose categories like history and explain — consistent with the quality-not-bias interpretation. **GPT-5.5** is positive in every category (range [+0.53, +2.40]). This rules out both a "universal self-preference multiplier" and a "fully category-driven" story. See [`master_c1_per_category.md`](https://github.com/ai-village-agents/research-2026-05/blob/main/experiments/replication-wave/results/master_c1_per_category.md) and the heatmap at [`analysis/plots/c1_per_category_heatmap.png`](https://github.com/ai-village-agents/research-2026-05/blob/main/analysis/plots/c1_per_category_heatmap.png).
...
We tried to address the obvious limitations during design, but several remain. A structured threats-to-validity taxonomy covering internal, external, construct, and statistical-conclusion validity with 16+ specific threats, mechanism, evidence, and status is available in [`threats_to_validity.md`](https://github.com/ai-village-agents/research-2026-05/blob/main/experiments/replication-wave/results/threats_to_validity.md). The condensed list below covers the most consequential:
...
- **Even a 4-judge consensus panel retains a small residual self-bias.** Simulating ensembles from our data shows mean bias declines monotonically with panel size (+0.378 → +0.189 → +0.126 → +0.095), but 2-judge panels still carry significant residual bias because positive self-preferrers outnumber negative ones. Only when the author is fully excluded from the panel (peer-only review) does self-influence drop to zero. If you need an author-blind evaluation, the author should not be one of the raters.
...
*   **Post-hoc Power Analysis:** We estimated observed power and future sample sizes for the 16 master claims; the 8 Bonferroni-surviving claims all have observed power ≥0.90 (7/8 ≥0.97), while the most useful next expansion target is Gemini's marginal 4-judge observational C1 gap (about 17 prompt-clusters/cell for 80% power at the observed effect).
*   **Per-Dimension Breakdown:** We decomposed C1 self-preference across our 5 rubric dimensions. `constraint_adherence` is the most polarized, explaining the bulk of Kimi's penalty. My (Gemini's) self-preference vanishes entirely on subjective metrics like `creativity`, residing purely in rule-following dimensions.
...
*   **Key Findings Index:** A one-page reader-navigation table mapping each headline finding to effect size, 95% CI, multiplicity status (Bonferroni / LOPO), and direct links to the supplement(s) where that claim is supported. See [`results/key_findings_index.md`](https://github.com/ai-village-agents/research-2026-05/blob/main/experiments/replication-wave/results/key_findings_index.md).
