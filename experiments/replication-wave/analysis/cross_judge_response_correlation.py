...
Tests whether the 3 native judges (Claude Opus 4.7, Gemini 3.1 Pro, GPT-5.5)
...
# Rows: only keep (pid,author) keys with all 3 judges scored
rows = []
for k in all_keys:
    pid, author = k
    if author == "unknown":
...
pairs = [("claude","gemini"), ("claude","gpt"), ("gemini","gpt")]
pair_results = []
for a, b in pairs:
...
with csv_path.open("w", newline="") as f:
    ...
    w.writerow(["prompt_id", "author", "claude", "gemini", "gpt"])
    for r in rows:
        w.writerow([r["prompt_id"], r["author"], f"{r['claude']:.4f}", f"{r['gemini']:.4f}", f"{r['gpt']:.4f}"])
...
md.append(f"N responses with all 3 judges scored: **{len(rows)}**.\n")
...
md.append("| author | claude | gemini | gpt |")
md.append("|--------|--------|--------|-----|")
for a in AUTHORS:
    md.append(f"| {a} | {author_judge[a]['claude']:.3f} | {author_judge[a]['gemini']:.3f} | {author_judge[a]['gpt']:.3f} |")
...
md.append("\n## 5. Interpretation\n")
md.append(
    f"The three native judges show **mean pairwise Spearman ρ = {mean_sp:.3f}** on per-response "
    f"composite scores across {len(rows)} responses. Restricting to non-self displayed labels "
    f"only gives ρ = {mean_sp_ns:.3f} (n={len(rows_ns)}). At the author level (4 authors × 3 "
    ...
    "of the blogpost (Gemini +0.293 [+0.142, +0.452]; Claude +0.120 [−0.067, +0.304]; GPT-5.5 "
    "label-invariant), this pattern is consistent with the 'biased, not noisy' interpretation: "
    "the three native judges substantially agree on *which* responses are better, and "
    "the residual self-preference signal is an additive bias on top of a shared quality "
    "signal rather than disagreement about quality itself."
)
