...
# Load data
df = pd.read_csv('experiments/replication-wave/results/long_scores.csv')
df['composite'] = df[['correctness', 'completeness', 'clarity', 'creativity', 'constraint_adherence']].mean(axis=1)

results = []

for judge in df['judge'].unique():
    judge_df = df[df['judge'] == judge]
    pivot_df = judge_df.pivot_table(
        index=['author', 'prompt_id'],
        columns='condition',
        values='composite',
        aggfunc='first'
    ).reset_index()
    
    c1_c2_rho = pivot_df['c1'].corr(pivot_df['c2'], method='spearman')
    c1_c3_rho = pivot_df['c1'].corr(pivot_df['c3'], method='spearman')
    c1_c3_mad = np.abs(pivot_df['c1'] - pivot_df['c3']).mean()
    c1_c2_mad = np.abs(pivot_df['c1'] - pivot_df['c2']).mean()
    
    results.append({
        'judge': judge,
        'c1_c2_spearman': c1_c2_rho,
        'c1_c3_spearman': c1_c3_rho,
        'c1_c2_mad': c1_c2_mad,
        'c1_c3_mad': c1_c3_mad
    })

results_df = pd.DataFrame(results)
results_df.to_csv('experiments/replication-wave/results/cross_condition_stability.csv', index=False)

md_content = """# Cross-Condition Score Stability (Post-v1.3.0 Exploratory Supplement)
...
with open('experiments/replication-wave/results/cross_condition_stability.md', 'w') as f:
    f.write(md_content)

print("Generated cross_condition_stability files.")
