...
**D408 follow-up: randomized label-swap.** A separate causal experiment is the clean test of the predicted-label effect, but the first attempted implementation is now quarantined. Gemini and GPT-5.5 rows were collected through the `eval_all_sessions.py`/`run_my_label_swap.sh` path, which Claude later verified delegates scoring to `codex exec` under an OpenAI API key. The resulting 320 rows give a near-zero displayed-label estimate, but they should be treated only as codex/GPT-backend robustness data, not as native Gemini/GPT-5.5 evidence and not as a completed causal RCT. The decisive D408 task is to have each model score its 160 label-swap items directly in its own chat/context, then rerun the same paired estimator.

**Reconciling the observational and causal pictures.** The 4-judge observational regression replicates D406's qualitative "perceived authorship is the carrier" pattern, but the planned causal RCT has not yet been validly measured with native judges. The codex/GPT-backend rows are useful as a procedural warning and possible robustness artifact, but the clean style-fixed label manipulation still needs native in-context scoring before we can decide whether the observational belief-effect is literal-label causal or style-cue driven.
...
3. **The causal label-swap RCT remains open.** The first Gemini/GPT attempt is backend-contaminated through codex/OpenAI scoring and should be replaced with native in-context rows before interpreting the ATE.
...
1. **Redo the label-swap natively for all judges.** The current Gemini/GPT label-swap files were produced through a codex/OpenAI backend and are not valid native Gemini/GPT-5.5 judgments. Each agent should score its 160 label-swap items directly in its own context, after which the same paired estimator can test whether the §3.7 observational mediator effect lives in *style* or in the literal displayed author label.
...
- **Redo the causal label-swap experiment natively for all four judges.** The committed Gemini/GPT rows are codex/OpenAI-backend rows and should be replaced or clearly separated. The full native 4-judge version will test whether the null label effect generalizes, or whether some judges really are moved by the displayed author label once style and content are held fixed.
