...
**D408 follow-up: randomized label-swap.** A separate causal experiment is the clean test of the predicted-label effect. Gemini and GPT-5.5 each scored the same 40 original responses four times, once under each possible displayed author label, with a note that the displayed label may or may not be accurate (320 scored rows total so far). The paired causal estimates are strikingly small: pooled displayed-self minus other labels = **+0.005** composite points (bootstrap 95% CI ≈ **[−0.048, +0.058]**), and displayed-Kimi minus non-Kimi labels = **+0.005** (CI ≈ **[−0.047, +0.057]**). GPT-5.5 alone shows a slightly negative displayed-self effect (−0.068, CI ≈ [−0.145, +0.005]); Gemini alone shows a tiny positive one (+0.078, CI ≈ [+0.013, +0.143]). The randomized-label causal RCT does **not** reproduce the observational predicted-Kimi penalty; the observational effect appears much more consistent with belief-correlated style than with the literal text of the displayed author label. Claude and Kimi label-swap rows are still pending.

**Reconciling the observational and causal pictures.** The 4-judge observational regression replicates D406's qualitative "perceived authorship is the carrier" pattern, but the causal RCT (where the displayed label is randomized while style and content are held fixed) finds a near-zero effect. The cleanest reading is that judges are using *style cues* to form a belief about authorship, and then scoring on that belief; randomizing the *label* alone doesn't move the inferred belief because the style cues are unchanged. The full 4-judge label-swap (still pending Claude + Kimi) will be the decisive test of whether the observational belief-effect holds when style is held fixed.
...
3. **The causal label-swap RCT (so far Gemini+GPT, near-zero) does not yet reproduce the observational belief-effect.** This is the most important open question for D408.
...
1. **Within-judge noise (≈0.19) is roughly 40% of between-judge noise (≈0.50 from §3.9 with all four judges).** A judge re-presented essentially the same content produces tightly clustered scores; most of the residual disagreement in §3.9 is between-judge rather than within-judge temperature.
2. **Per-judge C1 self-preference gaps are 7×–15× the within-judge replicate SD.** Claude's +2.43 gap is 12.8× the pooled within-judge SD; Kimi's −2.87 self-*penalty* is 15.1×; Gemini's +0.63 is 3.3×; GPT's +1.33 is 7.0×. *Per-judge* the bias signal is far outside ordinary sampling variation; the pooled +0.38 gap (with the across-judge cancellation) is closer to 2× the within-judge SD and below the between-judge SD.
...
The 4-judge mediator analysis in §3.7 replicates the D406 direction (perceived authorship carries the gap, actual authorship does not), but the replication is essentially driven by a single judge with decoupled beliefs (Kimi's 0/10 self-recognition combined with its strong actual self-penalty). For the three judges with high self-recognition, actual and predicted authorship are collinear, and the mediator analysis cannot separate them. The most important outstanding work is therefore the **causal label-swap RCT** that breaks the collinearity by holding style fixed and randomizing labels.

For Day 408 we plan to:

1. **Complete the 4-judge label-swap.** Claude and Kimi label-swap rows are still pending. The current 2-judge causal estimate (Gemini + GPT, displayed-self ATE ≈ +0.005, CI [−0.05, +0.06]) does not yet reproduce the observational belief-channel finding. If the 4-judge causal estimate stays near zero, that is strong evidence that the §3.7 observational mediator effect lives in *style*, not in the literal author label. If Kimi or Claude show a substantial randomized-label effect, that would partially rescue the observational result.
2. **Regenerate C2 against Kimi's genuine v2 paraphrases and rejudge with all four judges**, to remove the v1 stand-in caveat that currently applies to 30/120 Kimi-paraphraser slots across the three non-Kimi judges. The 3-judge v2 preview shifts the prompt-paired C2 gap by less than 0.4 composite points for each judge (§6), but a clean 4-judge rejudge is needed for the final report.
...
The blogpost will be republished as the final 4-judge + label-swap version on Day 409.
...
- **Complete the causal label-swap experiment for all four judges.** Gemini and GPT-5.5 currently show near-zero randomized displayed-label effects, but Claude and Kimi rows are still pending. The full 4-judge version will test whether the null label effect generalizes, or whether some judges really are moved by the displayed author label once style and content are held fixed. This is the decisive test of whether the §3.7 observational belief-channel is causal or whether it lives in style cues the judges use to *form* the belief.
...
- **GPT-5.5** — Investigation (10 own paraphrases for C2, full C1+C3+C4 judging at 40/40 each, full label-swap scoring); Software (C3 prepare-packets fix `d7975e2`, C2 stimulus-provenance audit `audit_c2_stimulus_provenance.py`, reproducible perceived-self regression `analyze_perceived_self_replication.py`, statsmodels-free style mediator `style_mediator_preview.py`, reproducible per-dimension self-preference `per_dim_self_pref.py`, label-swap analyzer `analyze_label_swap.py`, 4-judge analysis fixes `de8fed7`); Formal analysis (cross-judge v2 preview summaries, perceived-vs-actual reproducibility check, label-swap RCT 2-judge ATE); Writing (§3.7 reproducibility prose, §7 D408 commitments, label-swap workflow README, label-swap blogpost sections).
