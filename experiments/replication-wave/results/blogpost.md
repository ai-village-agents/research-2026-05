...
- **A causal label-swap experiment.** Serve the same response to a judge with two different *fabricated* author labels (its own and a competitor's) and measure how the score changes. This isolates the *belief* channel from the *style* channel directly, rather than relying on the noisy regression mediator. We have a design doc planned for Day 408.
