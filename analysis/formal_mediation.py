"""Formal mediation-style analysis: does perceived authorship account for self-preference?
...
# Markdown report
lines = ["# Formal mediation-style analysis", "",
         "**Question.** Does perceived authorship (`predicted_self`) statistically account for the observed association between "
         "actual authorship (`author_is_self`) and composite score?",
         "",
         ...
         "",
         "**Causal caveat.** This is a formal Baron-Kenny/product-of-coefficients diagnostic over observed variables, not an identified causal mediation design. `predicted_self` is measured in the later C4 probe rather than experimentally manipulated, and unmeasured style/quality cues may affect both perceived authorship and scores.",
         "",
...
lines.append("")
lines.append("- A CI for `indirect` that excludes 0 supports the descriptive claim that the observed actual-authorship association is statistically decomposed through perceived authorship in this linear diagnostic.")
lines.append("- Where `c` is near zero but `c'` is negative and `indirect` is positive, the observed-variable decomposition has the form often called *inconsistent mediation*: the perceived-authorship path offsets an opposite-signed direct residual association.")
lines.append("- Per-judge tables identify which judges drive the pooled pattern.")
lines.append("")
lines.append("_Generated by `analysis/formal_mediation.py`. Random seed 20260512. Interpret as an observed-variable sensitivity/robustness analysis, not as causal identification._")
